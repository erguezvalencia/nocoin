...
*Setup instructions for a Gitian build of Nocoin using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the Nocoin
...
- [Building Nocoin](#building-nocoin)
...
Clone the git repositories for nocoin and Gitian and then checkout the nocoin version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/nocoin-project/nocoin.git
cd nocoin
...
Follow the instructions in [doc/release-process.md](release-process.md) in the nocoin repository
...
To build Nocoin (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md) in the nocoin repository.
...
Output from `gbuild` will look something like
    ...
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/nocoin/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/nocoin-project/nocoin
...
For example:
```bash
URL=https://github.com/user/nocoin.git
COMMIT=2014_03_windows_unicode_path
./bin/gbuild --commit nocoin=${COMMIT} --url nocoin=${URL} ../nocoin/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit nocoin=${COMMIT} --url nocoin=${URL} ../nocoin/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit nocoin=${COMMIT} --url nocoin=${URL} ../nocoin/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/nocoin-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/nocoin-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/nocoin-build.assert
```
...
[nocoin/gitian.sigs](https://github.com/nocoin-project/gitian.sigs.ltc/) repository, or if that's not possible create a pull
request. 
